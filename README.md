...
6. **검수** — Codex(또는 Sonnet 5 폴백) 결과가 나오면 Claude가 diff를 직접 읽고 요구사항 충족 여부, 버그, 컨벤션 위반, 보안 이슈를 확인합니다. 가능하면 테스트/빌드도 돌려봅니다.
...
또는 슬래시 명령 없이 "코드 짜줘", "기능 구현해줘" 같은 실제 구현 요청에도 자동으로 트리거됩니다. 첫 실행 시:
