...
**명시 호출 전용입니다.** `/codex-dev`를 직접 입력했을 때만 실행되며, "코드 짜줘" 같은 일반 구현 요청만으로는 자동 발동하지 않습니다(그런 요청은 Claude가 직접 구현합니다). 프론트매터의 `disable-model-invocation: true`로 강제되므로, 모델이 스스로 이 스킬을 호출하는 경로 자체가 없습니다. 첫 실행 시:
